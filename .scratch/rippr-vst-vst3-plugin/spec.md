...
7. As a beat maker, I want the complete supported source acquired without configured duration or file-size ceilings, so that the tool does not reject media that the bundled tools and available disk can handle.
8. As a beat maker, I want an accurate waveform derived from the rendered WAV, so that the UI reflects the audio I will drag into the DAW.
9. As a beat maker, I want to choose where friendly-named WAVs are saved, so that completed samples appear in a folder I control.
10. As a beat maker, I want the chosen sample folder remembered, so that I do not have to select it for every rip.
11. As a beat maker, I want to audition previewable samples while oversized media remains downloadable and draggable, so that a large source cannot destabilize the DAW process.
...
28. As a beat maker, I want duplicate requests for the same source and output format to reuse a valid cached artifact, so that repeated work is fast and bandwidth-efficient.
29. As a beat maker, I want to see source, title, creator, duration, creation time, waveform, and file status for each Library Entry, so that I can identify and audit cached samples.
...
31. As a beat maker, I want an explicit clear-cache action, so that I can reclaim disk space intentionally without a hidden quota rejecting acquisitions.
...
34. As a beat maker, I want the DAW project to restore the Active Sample reference and plug-in parameters, so that reopening a session returns to the same setup.
...
41. As a keyboard user, I want URL entry, buttons, library navigation, focus states, and native edit shortcuts such as Command-V to be accessible, so that the editor is operable without precise pointer input.
...
- Use per-job temporary directories and atomically publish a completed artifact only after validation succeeds. Cancellation, worker crash, parse failure, disk failure, and media failure must remove or quarantine incomplete artifacts without damaging previously completed Library Entries.
- Select the provider's best available audio stream and render the complete source as lossless stereo 32-bit float PCM at its native sample rate in an RF64-capable WAV. Preserve source level by default, record the source and rendered sample rates in the manifest, and perform transcode work in the Worker.
...
- Expose output gain as the only automatable audio parameter in the first release. URL, metadata, library selection, job state, destination folder, and cache paths are non-automatable application state.
- Use a content-addressed Sample Library keyed by the acquisition identity, provider/source identity, output format version, and toolchain version. Store human metadata separately from immutable media identity so title edits do not duplicate audio.
- Persist Library metadata and waveform peaks in an embedded SQLite database configured for multi-instance access and transactional migrations. Store immutable media in a dedicated application-data cache without an artificial quota, and materialize friendly WAVs in the user's chosen folder.
- Persist only the Active Sample identifier, provenance needed to explain it, editor state, and automatable parameters in the DAW project state. Do not embed media bytes in VST3 state. On restore, load from the cache asynchronously; if missing, remain silent and require explicit reacquisition.
...
- Extend that same seam with scenarios for cancellation, worker exit, corrupt output, duplicate-cache reuse, missing-cache restoration, oversized-preview fallback, two independent plug-in instances, and project state round trips. Keep live provider/network behavior out of deterministic CI.
- Test the Worker as a black-box executable using local fixture media and a fake acquisition executable. Assert the versioned JSON-lines request/event contract, full-source direct argument passing, cancellation, temporary-file cleanup, and atomic publication.
...
- Test the frontend against a mock implementation of the same typed bridge. Assert the user-visible URL, metadata, real waveform, destination folder, progress, cancellation, error, library, missing-sample, drag fallback, focus, and restoration flows without coupling tests to DOM structure or CSS class names.
